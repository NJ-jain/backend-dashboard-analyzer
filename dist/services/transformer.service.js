...
exports.parseKPIStats = parseKPIStats;
exports.parseIncidentBreakdown = parseIncidentBreakdown;
exports.parseIncidentTrend = parseIncidentTrend;
exports.parseMSIParameters = parseMSIParameters;
exports.parseGembaData = parseGembaData;
exports.parseCriticalIssues = parseCriticalIssues;
exports.parseBodyPartInjuries = parseBodyPartInjuries;
exports.parseCommitteeMeetings = parseCommitteeMeetings;
exports.parseTrainingSessions = parseTrainingSessions;
exports.parseAuditCompliance = parseAuditCompliance;
exports.parseMockDrills = parseMockDrills;
exports.parseFlatSheetRow = parseFlatSheetRow;
exports.transformSheetWebhook = transformSheetWebhook;
/**
 ...
 */
function parseIncidentBreakdown(rows) {
    if (!Array.isArray(rows))
        return [];
 ...
 */
function parseIncidentTrend(rows) {
    if (!Array.isArray(rows))
        return [];
    return rows.map((row) => {
        return {
 ...
 */
function parseMSIParameters(rows) {
    if (!Array.isArray(rows))
        return [];
    return rows.map((row) => {
 ...
 */
function parseCriticalIssues(rows) {
    if (!Array.isArray(rows))
        return [];
    return rows.map((row, idx) => {
        ...
        if (lowerStatus.includes("op"))
            status = "Open";
        else if (lowerStatus.includes("prog") || lowerStatus.includes("work"))
            status = "In Progress";
        else if (lowerStatus.includes("close") || lowerStatus.includes("done") || lowerStatus.includes("resolv"))
            status = "Closed";
        return {
            id,
            issue,
 ...
 */
function parseBodyPartInjuries(rows) {
    if (!Array.isArray(rows))
        return [];
 ...
 */
function parseCommitteeMeetings(rows) {
    if (!Array.isArray(rows))
        return [];
    return rows.map((row) => {
 ...
 */
function parseTrainingSessions(rows) {
    if (!Array.isArray(rows))
        return [];
    return rows.map((row) => {
        return {
 ...
 */
function parseAuditCompliance(rows) {
    if (!Array.isArray(rows))
        return [];
    return rows.map((row) => {
 ...
 */
function parseFlatSheetRow(row, currentStore) {
    ...
    const msiParameters = msiMap.map(item => {
    ...
    if (criticalIssues.length === 0) {
        criticalIssues.push({ id: 1, issue: "Non-compliance with PTW procedure", area: "Work Area C", status: "Open" }, { id: 2, issue: "Improper storage of flammable chemicals", area: "Chemical Store A", status: "Open" });
    }
    ...
    const milestones = [];
    if (row.achievement1)
        milestones.push(safeString(row.achievement1));
    if (row.achievement2)
        milestones.push(safeString(row.achievement2));
    if (milestones.length === 0) {
        milestones.push("ZERO LTI for 3 consecutive months - 91 LTI-free days achieved", "MSI Score improved by 8 points vs previous month");
    }
    return {
        facility,
        region,
 ...
 */
function transformSheetWebhook(payload, currentStore) {
    const sheetName = safeString(payload.sheetName).toLowerCase();
    const rows = payload.sheetData || [];
    const firstRow = rows[0];
    const isFlatFormat = firstRow && ("ehsLead" in firstRow ||
        "nearMiss" in firstRow ||
        "headInjuries" in firstRow ||
        "finalMSIScore" in firstRow ||
        "siteGM" in firstRow);
    if (isFlatFormat) {
        console.log(`[Transformer] Flat high-density EHS sheet format detected in payload (sheet: "${payload.sheetName}"). Processing row...`);
        try {
            return parseFlatSheetRow(firstRow, currentStore);
        }
        catch (err) {
            console.error("[Transformer Error] Failed to parse flat EHS sheet row:", err);
            return currentStore;
        }
    }
    const updatedData = { ...currentStore };
    try {
        if (sheetName.includes("kpi") || sheetName.includes("stats")) {
            const parsedKpi = parseKPIStats(rows);
            if (parsedKpi.length > 0)
                updatedData.kpiStats = parsedKpi;
        }
            ...
            if (parsedIssues.length > 0)
                updatedData.criticalIssues = parsedIssues;
        }
            ...
            if (parsedBody.length > 0)
                updatedData.bodyPartInjuries = parsedBody;
        }
            ...
            if (parsedMeetings.length > 0)
                updatedData.committeeMeetings = parsedMeetings;
        }
        else if (sheetName.includes("training") || sheetName.includes("learn")) {
            const parsedTraining = parseTrainingSessions(rows);
            if (parsedTraining.length > 0)
                updatedData.trainingSessions = parsedTraining;
        }
            ...
            if (parsedMilestones.length > 0)
                updatedData.milestones = parsedMilestones;
        }
